...
PRIVATE int use_worst_fit; /*if it is set to 1 allocate mem using worst fit alghoritm
                            * if it is set to 0 allocate mem using standard algorithm*/
...
PUBLIC int do_hole_map(){
  ...
  while(hp != NIL_HOLE && hp->h_base && i < (nbytes - 1)){
  buff[i++] = hp->h_len;
  buff[i++] = hp->h_base;
  hp = hp->h_next;
  npairs++;
